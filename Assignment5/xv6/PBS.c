...
int getMinPrio() {
    ...
    for (struct proc *aProc = ptable.proc; aProc < &ptable.proc[NPROC]; aProc++) {
        if (aProc->state == RUNNABLE && aProc->priority < minProc) {
            minProc = aProc->priority;
        }
    }
    return minProc;
}
...
scheduler(void) {
    ...
    for (;;) {
        ...
        acquire(&ptable.lock);
        int currMinPrio = getMinPrio();

        // start round robin
        for (struct proc *aProc = ptable.proc; aProc < &ptable.proc[NPROC]; aProc++) {
            if (aProc->state == RUNNABLE && aProc->priority == currMinPrio) {
                p = aProc;
                c->proc = p;
                switchuvm(p);
                p->state = RUNNING;
