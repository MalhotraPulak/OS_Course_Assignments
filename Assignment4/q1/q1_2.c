...
void mergeSort(int *a, int l, int r, int n) {
    if (r - l + 1 < 5) {
        ...
        /* fork a child process, this child merge sort the left half */
        int child_pid = fork();
        if (child_pid < 0) {
        ...
        if (child_pid == 0) {
            ...
            int t = write(pipeL[WRITE_END], a, n * sizeof(int));
            assert(t == 4 * n);
            _exit(0);
        }
        /* in parent now */
        ...
        if (child_pid_2 == 0) {
            ...
            mergeSort(a, m + 1, r, n);
            int t = write(pipeR[WRITE_END], a, n * sizeof(int));
            assert(t == 4 * n);
            _exit(0);
        }
        ...
        for (int k = 0; k < 2; k++) {
            ...
            if (kill_id == child_pid) {
                int t = read(pipeL[READ_END], returned_data, n * sizeof(int));
                assert(t == 4 * n);
                close(pipeL[0]);
                for (int i = l; i <= m; i++) {
                    a[i] = returned_data[i];
                ...
                int t = read(pipeR[READ_END], returned_data, n * sizeof(int));
                assert(t == 4 * n);
                close(pipeR[0]);
                for (int j = m + 1; j <= r; j++) {
                    a[j] = returned_data[j];
